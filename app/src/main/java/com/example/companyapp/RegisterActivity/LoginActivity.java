...
import com.example.companyapp.UserDetailsForm.BusinessRelatedData1;
...
public class LoginActivity extends AppCompatActivity {
            ...
            login.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (TextUtils.isEmpty(etemail.getText().toString()) ||
                        ...
                        logiUsers(loginRequest,loginResponce);
    ...
    private void logiUsers(LoginRequest loginRequest,LoginResponce loginResponce) {
        Call<LoginResponce> loginResponceCall=ApiClient.getService().loginUser(sendToken,loginRequest);
        loginResponceCall.enqueue(new Callback<LoginResponce>() {
            @Override
            public void onResponse(Call<LoginResponce> call, Response<LoginResponce> response) {
                String token = response.headers().get("Token");
                if (response.isSuccessful()){
                    sessionManager.saveAuthToken(loginResponce.getAuthToken());
